...
void drawSkeleton(int userId)
{
  ...
  println(com2d.x);
  torsoX = map(com2d.x,0,750,0,400);
  torsoY = map(com2d.y,-200,600,0,300);
  
  PVector cuelloPos = new PVector();
  context.getJointPositionSkeleton(userId,SimpleOpenNI.SKEL_HEAD,cuelloPos);
  //println(cuelloPos.y);
  cabezaX = map(cuelloPos.x,-540,680,0,400);
  cabezaY = map(cuelloPos.y,600,-100,0,300);
  ...
  manoIzqY = map(manoIzqPos.y,1000,-600,0,300);
  ...
  manoDerX = map(manoDerPos.x,-500,500,0,400);
  manoDerY = map(manoDerPos.y,900,-600,0,300);
